-- ============================================================
-- Patch: fix merge_events() verification re-pointing
--
-- The straight UPDATE on event_verifications fails when both
-- the canonical and duplicate event already have a verification
-- row for the same URL (unique constraint on
-- event_id + source_url_normalized).
--
-- Fix: INSERT ... ON CONFLICT DO NOTHING, then DELETE.
-- If canonical already has a row for that URL, the duplicate's
-- row is redundant — skip it. The confidence recompute in step 8
-- sees the canonical's existing verification, so no signal is lost.
-- ============================================================
...
BEGIN
  ...
  -- 3. Verifications (INSERT + DELETE to handle UNIQUE(event_id, source_url_normalized))
  --    If canonical already has a row for the same URL, the duplicate's row is
  --    redundant — DO NOTHING. The confidence recompute sees canonical's existing row.
  INSERT INTO event_verifications
    (event_id, source_url, source_type, trust_weight, verified_fields, verified_by, verified_at, created_at)
  SELECT
    p_canonical_id, source_url, source_type, trust_weight, verified_fields, verified_by, verified_at, created_at
  FROM event_verifications
  WHERE event_id = p_duplicate_id
  ON CONFLICT ON CONSTRAINT uq_event_verifications_normalized DO NOTHING;

  DELETE FROM event_verifications WHERE event_id = p_duplicate_id;
