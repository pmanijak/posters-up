-- Migration: stricter null-date handling in run_dedup_pass fuzzy tier
--
-- Problem: when both events have null date_start, the date filter was a
-- no-op (NULL OR NULL = true), leaving name similarity as the only gate.
-- This caused N² false-positive explosions among recurring or multi-flyer
-- events — e.g. five null-date "Rad Pride" variants all matching each other.
--
-- Fix: three-way date condition in the fuzzy tier:
--   1. Both dates known and within 1 day (unchanged)
--   2. One date known, one unknown — name/location carries the signal (unchanged)
--   3. Both dates unknown — require name similarity >= 0.85 instead of 0.65
