-- Migration: add event name similarity gate to talent_anchor tier in run_dedup_pass
--
-- Problem: a shared billing_position=1 act caused "One Day Closer to Doom"
-- to incorrectly merge with "Storm Boy / 30 Coffin / The Pesaffits", two
-- completely unrelated events that happened to share a headliner.
--
-- Fix: require event names to be at least loosely similar (>= 0.30) before
-- a talent anchor match can fire. Low enough to allow name variation across
-- extractions; high enough to block merges between clearly different events.
