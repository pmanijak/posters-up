...
BEGIN
  ...
  FOR v_cluster IN SELECT * FROM _clusters LOOP
    ...
    -- common inside real band names (Peter & the Wolverines, Shannon
    -- and the Clams, Morgan & The Organ Donors), so their presence
    -- can't safely EXEMPT a name from the word-count check -- tested
    -- against this exact dataset, doing so would silently un-flag
    -- "Owl & The Pussycat Slippers" (a real concatenation -- "Slippers"
    -- is independently a distinct act on the same lineup) and "A-Ron
    -- Gubbe & The Zips" (glued onto the separately-flagged real band
    -- "Gubbe & The Zips"). Surfaced in `detail` instead, so a human
    -- reviewing the queue can quickly deprioritize "5 words, but only
    -- 2 once you strip the conjunction" against "5 words, still 5
    -- stripped" -- the latter is much more likely real contamination.
    ...
    IF v_anchor_id IS NULL AND (v_word_count >= v_word_threshold OR v_has_dash) THEN
    ...
    IF v_anchor_id IS NULL THEN
      ...
      RETURN NEXT;
    ELSE
      IF v_word_count >= v_word_threshold OR v_has_dash THEN
