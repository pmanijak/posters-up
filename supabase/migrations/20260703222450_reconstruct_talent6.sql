  ...
  FOR v_cluster IN SELECT * FROM _clusters LOOP
    ...
    ELSE
      IF (v_word_count >= v_word_threshold OR v_has_dash) AND NOT v_is_resolved_real THEN
        -- Real cases this catches: an event-format prefix glued onto a
        -- performer name via a dash, consistent across every sighting of
        -- each flyer, so no corroboration check could have caught either one.
        -- Skips resurrect/rename
